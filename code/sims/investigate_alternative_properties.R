...
source(here(code_prefix, "investigate_alternative_once.R"))
...
parser$add_argument("--j", default = 2, type = "double",
...
parser$add_argument("--bootstrap", default = 1, type = "double",
...
system.time(sim_output <- sapply(
  ...
  function(i) {
    do_one(iteration = i + args$nreps_per_job * (current_dynamic_args$mc_id - 1),
           ...
           linkfun = "probit", sl_lib = learner_lib, V = V,
           bootstrap = as.logical(args$bootstrap), b = b)
  }, simplify = FALSE)
)
...
sim_output_tib %>%
  mutate(bias_init = sqrt(n) * (est - truth),
         cover_init = cil <= truth & truth <= ciu) %>%
  group_by(type, j) %>%
  summarize(mn_est = mean(est), true = mean(truth),
            bias = mean(bias_init), cover = mean(cover_init))
