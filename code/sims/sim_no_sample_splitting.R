...
## ------------------------------------------------
## FILE: sim_no_sample_splitting.R
## CREATED: 31 March 2020 by Brian Williamson
## PURPOSE: simulation with binary outcome, binary predictor
## and no sample splitting, delta = 0.05
## ------------------------------------------------
...
# install.packages("~/Projects/UW/vimp_2.0.1.tar.gz", repos = NULL, type = "source")
library("vimp")
library("methods")
library("argparse")
library("xgboost")
...
if (!is.na(Sys.getenv("RSTUDIO", unset = NA))) { # if running locally
  ...
  source("lowdim/R/sim_no_ss_ests.R")
  source("lowdim/R/run_sim_no_ss_once.R")
} else {
  source("sim_binary_bivariate_data.R")
  source("sim_no_ss_ests.R")
  source("run_sim_no_ss_once.R")
}
...
parser$add_argument("--sim-name", default = "bivariate_loss_no_ss",
...
parser$add_argument("--vimp-measure", nargs = "+", default = "auc",
...
parser$add_argument("--delta", type = "double", default = 0.05,
                    help = "delta value for null testing")
...
## get the truths
truth <- truths %>%
  filter(type %in% args$vimp_measure)
...
## if sim name doesn't have "simple_lib" in it, use flexible library
if (!grepl("simple_lib", args$sim_name)) {
...
## ---------------------------------------------
current_seed <- current_dynamic_args$mc_id + current_dynamic_args$j + ifelse(args$risk_type == "expected_loss", 0, 1) + current_dynamic_args$n + job_id + 100*args$delta
print(current_seed)
set.seed(current_seed)
system.time(sim_output <- replicate(args$nreps_per_job,
    run_sim_no_ss_once(n = current_dynamic_args$n,
...
file_name <- paste0(args$sim_name, "_", paste0(args$vimp_measure, collapse = "_"), "_cv_", args$cv, "_delta_", args$delta, "_", job_id, ".rds")
