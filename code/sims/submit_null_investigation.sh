...
echo -e '#!/bin/bash\n Rscript investigate_null_properties.R' \
        '--sim-name $1 --learner $2 --nreps-total $3 ' \
        '--nreps-per-job $4 --cv $5 --j $6 --p $7' \
        '--corr $8 --bootstrap $9' > investigate_null.sh
chmod u+x investigate_null.sh
if [ ${11} -eq 0 ]; then
    sbatch -A gilbert_p --time=1-0 --array=$arry -e $io_file -o $io_file ./investigate_null.sh $1 $2 $3 $4 $5 $6 $7 $8 $9
else
    sbatch --qos=restart-new --partition=restart-new --time=1-0 --array=$arry -e $io_file -o $io_file ./investigate_null.sh $1 $2 $3 $4 $5 $6 $7 $8 $9
fi
rm investigate_null.sh
