...
sl_one_outcome <- function(dat, outcome_name, pred_names, save_dir = "slfits/",
                           cv_fit_name = paste0("cvfit_", outcome_name, ".rds"),
                           save_full_object = TRUE, reduce_covs = FALSE, ...){
  ...
  saveRDS(cv_fit$SL.predict, file = paste0(
    save_dir, gsub(".RData", ".rds", gsub("cvfit_", "cvfitted_", cv_fit_name))
  ))
  saveRDS(cv_fit$folds, file = paste0(
    save_dir, gsub(
      "cvfitted_", "cvfolds_", gsub(".RData", ".rds",
                                    gsub("cvfit_", "cvfitted_", cv_fit_name))
    )
  ))
  # non-cross-validated fit
  L$cvControl$V <- L$cvControl$V / 2
  L$cvControl$validRows <- NULL
  fit <- SuperLearner(Y = Y, X = preds, family = L$family, SL.library = L$SL.library,
                      method = L$method, cvControl = L$cvControl)
  saveRDS(fit, file = paste0(save_dir, gsub("cvfit_", "fit_", cv_fit_name)))
  saveRDS(fit$SL.predict, file = paste0(
    save_dir, gsub(".RData", ".rds", gsub("cvfit_", "fitted_", cv_fit_name))
  ))
...
SL.glmnet.1 <- function (Y, X, newX, family, obsWeights, id, alpha = 1, nfolds = 5,
...
descr_SL.glmnet.0 <- paste0(descr_SL.glmnet, "0")
descr_SL.glmnet.1 <- "GLMNET with lambda selected by CV and alpha equal to 1"

descr_SL.mean <- "intercept only regression"
descr_SL.glm <- "main terms generalized linear model"

library_full <- c("SL.mean", paste0("SL.xgboost", c(1, 2, 4, 6, 8)),
                  paste0("SL.ranger.", c("small", "reg", "large")),
                  paste0("SL.glmnet.", c(0, 25, 50, 75, 1)))
